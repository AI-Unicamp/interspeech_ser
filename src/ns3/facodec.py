...
class FACodecDecoderV2(nn.Module):
    ...
    def get_processed_style_embedding(self, x, prosody_feature, n_quantizers=None):
        """
        Process prosody feature through melspec and first quantizer only.
        
        Args:
            x: The encoder output (not used but kept for API consistency)
            prosody_feature: The prosody feature to process
            n_quantizers: Optional number of quantizers to use
            
        Returns:
            tuple: (out, q, commit, quantized) from f0 quantizer
        """
        # Process f0 input
        ...
        # Get first (prosody) quantizer output only
        f0_quantizer = self.quantizer[0]
        out, q, commit, quantized = f0_quantizer(f0_input, n_quantizers=n_quantizers)
        
        return out, q, commit, quantized

    def get_processed_style_speaker_embedding(self, x, prosody_feature, n_quantizers=None):
        """
        Process prosody feature and speaker feature through melspec and first quantizer only.
        ...
        x_timbre = x.transpose(1, 2)
        x_timbre = self.timbre_encoder(x_timbre, None, None)
        x_timbre = x_timbre.transpose(1, 2)

        result = torch.cat((out, x_timbre), dim=1)

        return out, q, commit, quantized, result
