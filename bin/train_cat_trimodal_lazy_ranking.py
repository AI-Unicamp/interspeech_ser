...
train_dataset = MultiLabelAudioDataset(train_df['FileName'].tolist(), train_df[classes].values, lazy_path1, lazy_path2, lazy_path3)
val_dataset = MultiLabelAudioDataset(val_df['FileName'].tolist(), val_df[classes].values, lazy_path1, lazy_path2, lazy_path3)



# if(balanced_batch):
logger.info(f'Using balanced batch neutral x non neutral. Computing sample weights...')

train_df["is_neutral"] = train_df['Neutral']
train_df["is_not_neutral"] = 1 - train_df['Neutral']
classes2 = ["is_neutral",'is_not_neutral']

class_frequencies = train_df[classes2].sum().to_dict()
...
sample_weights = [class_weights[train_df[classes2].iloc[i].idxmax()] for i in range(len(train_df))]
...
margin_loss = nn.SoftMarginLoss(reduction="mean")
...
for epoch in range(EPOCHS):
    ...
    for batch in tqdm(train_loader):
        ...
        y_neutral = batch['label']; y_neutral = y_neutral[:,-1]; y_neutral = y_neutral.cuda(non_blocking=True).long()
        
        y_neutral = 2*y_neutral - 1
        y_neutral = y_neutral.unsqueeze(-1)

        # break
        optimizer.zero_grad()
        outputs, logits_neutral = ser_model(inputs1, inputs2, inputs3)
        logits = outputs
        if(balanced_batch):
        ...
        soft_loss = margin_loss(logits_neutral, y_neutral)

        if(use_focalloss):
            
            floss = focal_loss(logits, y)

            total_loss = (floss + soft_loss) / ACCUMULATION_STEP
        else:
            total_loss = (loss+ soft_loss) / ACCUMULATION_STEP
