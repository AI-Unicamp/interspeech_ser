...
txt_opt = torch.optim.AdamW(text_model.parameters(), 1e-6, weight_decay=1e-1)
ssl_opt = torch.optim.AdamW(ssl_model.parameters(), 1e-6, weight_decay=1e-1)
...
for epoch in range(EPOCHS):
    ...
    if min_loss > dev_loss:
        ...
        torch.save(text_model.state_dict(), \
            os.path.join(MODEL_PATH,  "final_text_model.pt"))
