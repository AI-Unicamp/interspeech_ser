...
for epoch in range(EPOCHS):
    ...
    for xy_pair in tqdm(total_dataloader["train"]):
        ...
        # print(emo_pred.shape, y.shape)

        floss = focal_loss(emo_pred, y)
        ...
        if((batch_cnt+1)%200 == 0):
            logger.info(f"Epoch ({epoch+1}/{EPOCHS})| step = {batch_cnt}: ce loss = {loss} focal loss = {floss} total loss {total_loss}")
