...
class MultiModalEmotionClassifier(nn.Module):
    def __init__(
        ...
        # Layer normalization after projections (new)
        ...
        # Gating layers (new)
        self.speech_gate = nn.Linear(fusion_hidden_dim * 2, fusion_hidden_dim * 2)
        self.text_gate = nn.Linear(fusion_hidden_dim * 2, fusion_hidden_dim * 2)
        
        # Classifier (input is concatenated gated representations)
    ...
    def forward(self, features1, features2):
        ...
        # Apply gating mechanism (new)
        speech_gate = torch.sigmoid(self.speech_gate(speech_pooled))
        text_gate = torch.sigmoid(self.text_gate(text_pooled))
        
        speech_gated = speech_pooled * speech_gate
        text_gated = text_pooled * text_gate
        
        # Concatenate gated representations
        concatenated = torch.cat([speech_gated, text_gated], dim=-1)  # [batch, fusion_hidden_dim*4]
        
        # Layer norm and classify
        normalized = self.layer_norm(concatenated)
        logits = self.classifier(normalized)
        
        return logits, speech_gated, text_gated
...
for epoch in range(EPOCHS):
    ...
    for batch in tqdm(train_loader):
        ...
        if(use_focalloss):
            ...
            total_loss = (floss + loss_cka) / ACCUMULATION_STEP
        else:
        ...
        if((batch_cnt+1)%200 == 0):
            logger.info(f"Epoch ({epoch+1}/{EPOCHS})| step = {batch_cnt}: loss = {loss} cka = {loss_cka} current lr = {current_lr}")
    ...
    for batch in tqdm(val_loader):
        ...
        with torch.no_grad():
            ...
            total_wav.append(speech_pooled)
            total_rob.append(text_pooled)
    ...
    dev_loss = loss
    # Logging
    logger.info(f"|VALIDATION| Epoch ({epoch+1}/{EPOCHS}): eval_loss = {loss} eval f1 = {f1} cka eval = {loss_cka_eval}")
