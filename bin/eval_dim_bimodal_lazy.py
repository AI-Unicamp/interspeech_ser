...
dev_loss = loss
# Logging
logger.info(f"|Metrics| eval_loss = {ccc}")

def save_predictions_with_probs(total_pred, total_utt, model_path, dtype='dev'):
    data = []
    ...
    for pred, utt in zip(total_pred, total_utt):
        ...
        for i, prob in enumerate(pred):
            prob_ = min(max(1, prob * 6 + 1), 7)
            row.append(f'{prob_}')
        
        data.append(row)
    
    # Create headers for the CSV
    headers = ['Filename', "EmoAct_pred", "EmoDom_pred", "EmoVal_pred"]
...
csv = save_predictions_with_probs(total_pred, total_utt, MODEL_PATH, dtype='dev')