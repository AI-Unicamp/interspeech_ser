...
class EncoderDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        for item in batch:
            ...
            feat_name = os.path.join(self.lazy_path, utter_path.replace('.wav','.pt'))
...
num_classes_in_batch = 8
num_utter_per_class = 10
num_utter_per_class_val = 5


train_dataset = EncoderDataset(train_df, audio_lazy_path, num_classes_in_batch,num_utter_per_class)
val_dataset = EncoderDataset(val_df, audio_lazy_path, num_classes_in_batch,num_utter_per_class_val)
...
ser_model = WavLMSERClassifier(wavlm_dim=1024, hidden_dim=512, num_categories=8, num_attention_heads=1)
ser_model.to(device)
...
for epoch in range(EPOCHS):
    ...
    for batch in tqdm(train_loader):
        ...
        optimizer.zero_grad()
        embeddings ,outputs = ser_model(inputs)
        logits = outputs
        ...
        # Logging
        if((batch_cnt+1)%50 == 0):
            logger.info(f"Epoch ({epoch+1}/{EPOCHS})| step = {batch_cnt}: loss_ce = {loss_ce} angle = {loss_angle} current lr = {current_lr}")
