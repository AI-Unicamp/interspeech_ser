...
feature_extractor = AutoProcessor.from_pretrained('openai/whisper-large-v3')
...
for dtype in ["train", "dev"]:
    ...
    if dtype == "train":
        cur_wav_set = utils.WavSet(cur_wavs, normalize_wav=normalize_wav, use_tp = USE_TIMBRE_PERTURB, tp_prob= TP_PROB, processor = feature_extractor)
        ...
        cur_wav_set = utils.WavSet(cur_wavs, wav_mean=wav_mean, wav_std=wav_std, normalize_wav=normalize_wav, processor = feature_extractor)
    ########################################################
...
ser_model = WhisperAudioClassifier()
