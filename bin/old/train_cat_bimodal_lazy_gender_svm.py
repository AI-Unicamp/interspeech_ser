...
from src.modules import grl
import math
from torch.optim.lr_scheduler import _LRScheduler

class SVMRankingLoss(nn.Module):
    ...
    def forward(self, outputs, target):
        ...
        # Reshape outputs to compare male vs female scores
        male_scores = outputs[:, 1].view(-1, 1)
        female_scores = outputs[:, 0].view(-1, 1)
        
        # Create target matrix (1 for male, -1 for female)
        target_matrix = 2 * target.float() - 1
        
        # Calculate ranking loss
        diff = self.margin - target_matrix * (male_scores - female_scores)
...
df = df.merge(gender_labels[["FileName","Gender"]], on = 'FileName', how = 'left')

# Classes (emotions)
classes = ['Female', 'Male']
label_mapping = {name: idx for idx, name in enumerate(classes)}
df["target_gender"] = df["Gender"].map(label_mapping)
...
def collate_fn(batch):
    ...
    labels_gender = [item['gender'] for item in batch]
    ...
    stacked_labels_gender = torch.stack(labels_gender)  # (batch_size, num_labels)

    return {
        "feat1": padded_feats1,
        "feat2": padded_feats2,
        "label": stacked_labels,
        "gender": stacked_labels_gender
    }

class MultiLabelAudioDataset(Dataset):
    def __init__(self, wav_files, labels, gender_labels, lazy_path1, lazy_path2):
...
class MultiModalEmotionClassifier(nn.Module):
    def __init__(
        ...
        # Gender Classifier with Gradient Reversal
        self.gender_classifier = nn.Sequential(
            # grl.GradientReversal(lambda_reversal=1),
            nn.Linear(fusion_hidden_dim * 4, fusion_hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, features1, features2):
        ...
        gender_logits = self.gender_classifier(normalized)

        return logits, gender_logits

train_dataset = MultiLabelAudioDataset(train_df['FileName'].tolist(), train_df[classes].values, train_df['target_gender'].values, lazy_path1, lazy_path2)
val_dataset = MultiLabelAudioDataset(val_df['FileName'].tolist(), val_df[classes].values, val_df['target_gender'].values, lazy_path1, lazy_path2)
...
svm_ranking_loss = SVMRankingLoss(margin=1.0)
for epoch in range(EPOCHS):
    ...
    for batch in tqdm(train_loader):
        ...
        gender_y = batch["gender"].to(device).long()
        # print(inputs1.shape, inputs2.shape)
        # labels = batch["label"].to(device)
        y = batch['label']; y=y.max(dim=1)[1]; y=y.cuda(non_blocking=True).long()

        optimizer.zero_grad()
        outputs, gender = ser_model(inputs1, inputs2)
        logits = outputs
        if(balanced_batch):
            loss = utils.CE_weight_category(gender, gender_y, None)
        else:
            loss = utils.CE_weight_category(logits, y, class_weights_tensor)

        gender_loss = svm_ranking_loss(gender, gender_y)*0.01

        if(use_focalloss):
            
            floss = focal_loss(logits, y)

            total_loss = (floss + gender_loss) / ACCUMULATION_STEP
        else:
            total_loss = (loss + gender_loss) / ACCUMULATION_STEP
        ...
        if((batch_cnt+1)%200 == 0):
            logger.info(f"Epoch ({epoch+1}/{EPOCHS})| step = {batch_cnt}: loss = {loss} gender_loss = {gender_loss} current lr = {current_lr}")
    ...
    for batch in tqdm(val_loader):
        ...
        with torch.no_grad():
            ...
            total_pred_gender.append(gender)
            total_y_gender.append(gender_y)
    ...
    total_pred_gender = torch.cat(total_pred_gender, 0)
    total_y_gender = torch.cat(total_y_gender, 0)

    loss = utils.CE_weight_category(total_pred, total_y, class_weights_tensor_val)
    f1 = f1_score(val_labels, val_preds, average='macro')
    loss_gender = utils.CE_weight_category(total_pred_gender, total_y_gender, None)
   

    dev_loss = loss
    # Logging
    logger.info(f"|VALIDATION| Epoch ({epoch+1}/{EPOCHS}): eval_loss = {loss} eval f1 = {f1} eval gender CE = {loss_gender}")
