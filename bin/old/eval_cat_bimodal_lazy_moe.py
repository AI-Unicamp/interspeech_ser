...
class ExpertModule(nn.Module):
    def __init__(self, features1_dim, features2_dim, fusion_hidden_dim, num_emotions, dropout=0.5):
        super().__init__()
        
        # Core architecture similar to original model
        ...
        self.speech_gru = nn.GRU(fusion_hidden_dim, fusion_hidden_dim, batch_first=True, bidirectional=True)
        self.text_gru = nn.GRU(fusion_hidden_dim, fusion_hidden_dim, batch_first=True, bidirectional=True)
    ...
    def forward(self, features1, features2):
        speech_proj = self.speech_projection(features1)
        text_proj = self.text_projection(features2)
        
        speech_hidden, _ = self.speech_gru(speech_proj)
        text_hidden, _ = self.text_gru(text_proj)
        
        speech_attended, _ = self.speech_attention(speech_hidden, text_hidden, text_hidden)
        text_attended, _ = self.text_attention(text_hidden, speech_hidden, speech_hidden)
        ...
        concatenated = torch.cat([speech_pooled, text_pooled], dim=-1)
...
class MoEEmotionClassifier(nn.Module):
    def __init__(
        ...
        self.experts = nn.ModuleList([
            ExpertModule(features1_dim, features2_dim, fusion_hidden_dim, num_emotions, dropout)
            for _ in range(num_experts)
        ])
        
        # Expert gating network
        self.gate = nn.Sequential(
            nn.Linear(features1_dim + features2_dim, fusion_hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(fusion_hidden_dim, num_experts),
        )
        
        # Confidence network for each expert
    ...
    def forward(self, features1, features2):
        ...
        # Calculate gates (which experts to use)
        # Pool features for gate calculation
        f1_pooled = features1.mean(dim=1)
        f2_pooled = features2.mean(dim=1)
        gate_input = torch.cat([f1_pooled, f2_pooled], dim=-1)
        gates = F.softmax(self.gate(gate_input), dim=-1)
        
        # Get predictions from each expert
        expert_outputs = []
        for expert in self.experts:
            expert_out = expert(features1, features2)
            expert_outputs.append(expert_out)
        expert_outputs = torch.stack(expert_outputs, dim=1)  # [batch, num_experts, num_emotions]
        
        # Weight expert predictions by gates
        gates = gates.unsqueeze(-1)  # [batch, num_experts, 1]
        weighted_outputs = expert_outputs * gates
        
        # Sum weighted predictions
        final_output = weighted_outputs.sum(dim=1)  # [batch, num_emotions]
        
        return final_output

    def get_expert_contributions(self, features1, features2):
        """Helper method to analyze which experts are being used"""
        f1_pooled = features1.mean(dim=1)
        f2_pooled = features2.mean(dim=1)
        gate_input = torch.cat([f1_pooled, f2_pooled], dim=-1)
        gates = F.softmax(self.gate(gate_input), dim=-1)
...
ser_model = MoEEmotionClassifier(
