...
logger.info(f"Starting an Lazy OwnSermodel for dimensional output saving at = {MODEL_PATH}")
logger.info(f"Using LR = {LR} Epochs = {EPOCHS} Batch size = {BATCH_SIZE} Accum steps = {ACCUMULATION_STEP}")
logger.info(f"Using mse = {use_mse}")
...
class MultiLabelAudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = (self.labels[idx] -1 )/7

        if(self.verbose_one):
            print(feat_name1, feat_name2, label)
...
# Load weights, excluding the last layer
new_state_dict = ser_model.state_dict()
pretrained_state_dict = {
    k: v for k, v in checkpoint.items() if k in new_state_dict and v.size() == new_state_dict[k].size()
}
new_state_dict.update(pretrained_state_dict)
ser_model.load_state_dict(new_state_dict)

# ser_model.load_state_dict(torch.load(PRETRAINED_PATH, map_location='cpu'), strict=False)
ser_model.to(device)
...
for epoch in range(EPOCHS):
    ...
    for batch in tqdm(train_loader):
        ...
        y = batch['label']; y=y.cuda(non_blocking=True)

        optimizer.zero_grad()
        outputs = ser_model(inputs1, inputs2)


        ccc = utils.CCC_loss(outputs, y)
        loss = 1.0 - ccc
        loss = torch.sum(loss)

        if(use_mse):
            mse_loss = utils.MSE_emotion(outputs, y)*10

            total_loss = (mse_loss + loss) / ACCUMULATION_STEP
        ...
        # print(loss, ccc)
        # print(loss, mse_loss, total_loss)
        total_loss.backward()

        # #If you want to check gradients, check the model parameters instead:
        ...
        if((batch_cnt+1)%200 == 0):
            if(use_mse):
                logger.info(f"Epoch ({epoch+1}/{EPOCHS})| step = {batch_cnt}: loss = {loss} mse_loss = {mse_loss} current lr = {current_lr}")
            else:
    ...
    for batch in tqdm(val_loader):
        ...
        # y = batch['label']; y=y.cuda(non_blocking=True).long()
    ...
    # Logging
    logger.info(f"|VALIDATION| Epoch ({epoch+1}/{EPOCHS}): eval_loss = {ccc}")


    dev_loss = 3.0 - ccc[0] - ccc[1] - ccc[2]
